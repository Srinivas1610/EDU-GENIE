...
def check_and_reload_api_key():
    ...
    new_model = os.getenv("GEMINI_MODEL_NAME", "gemini-1.5-flash")
    ...
    if new_key and (not models.gemini_initialized or config.GEMINI_API_KEY != new_key):
        import google.generativeai as genai
        try:
            genai.configure(api_key=new_key)
            config.GEMINI_API_KEY = new_key
            models.gemini_initialized = True
            logger.info("Successfully loaded/updated Gemini API key on-the-fly.")
        except Exception as e:
            logger.error(f"Error configuring Gemini SDK on-the-fly: {e}")
...
async def read_dashboard(request: Request):
    ...
    return templates.TemplateResponse(
        ...
        name="index.html",
        context={
            "gemini_active": models.gemini_initialized,
            "local_model_name": config.LAMINI_MODEL_NAME
        }
    )
...
async def ask_question(data: QARequest):
    ...
    # Decide model based on user selection or key presence
    if data.use_local or not models.gemini_initialized:
        # Use local T5 model
        ...
        try:
            ...
            return {
                "answer": answer,
                "model_used": f"Local Model ({config.LAMINI_MODEL_NAME})",
                "success": True
            }
        except Exception as e:
...
async def explain_concept(data: ExplainRequest):
    ...
    if not models.gemini_initialized:
        raise HTTPException(
            status_code=400, 
...
async def summarize_text(data: SummarizeRequest):
    """Summarizes text locally using LaMini-Flan-T5."""
    # Local model is perfect for fast local text-to-text summarization
