document.addEventListener("DOMContentLoaded", () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (status.gemini_active) {
                    ...
                    geminiText.innerText = `Active (${status.local_model_name === 'gemini-3.5-flash' ? 'Gemini 3.5' : 'Gemini 2.0'})`;
                ...
                // Update Local Model Dot and Label
    ...
    formQA.addEventListener("submit", async (e) => {
        ...
        const isLocalActive = useLocal || document.getElementById("status-gemini-dot").classList.contains("inactive");
        ...
        showLoading(isLocalActive ? "Inference using local model..." : "Querying Gemini...");
    ...
    formSummarize.addEventListener("submit", async (e) => {
        ...
        const localDot = document.getElementById("status-local-dot");
        const localText = document.getElementById("status-local-text");
        if (!localDot.classList.contains("active")) {
            localDot.className = "status-dot loading";
            localText.innerText = "Downloading...";
        }

        showLoading("Summarizing text locally using T5...");
